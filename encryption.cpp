...
int main() {
	...
	if (type == 1){
        ...
        username part5 = username(getTextKey(part4.password), dateKey);
        cout << "Date Key: " << dateKey << endl << "Username: " << part2.theusername << endl << "Password: " << part5.theusername << endl;
	}
	if (type == 2) {
        textKey = getTextKey(message);
        dateKey = inputDate;
        ...
        monarch part4 = monarch(part3.targetKey);
        username part5 = username(getTextKey(part4.password), dateKey);
        cout << "Username: " << part2.theusername << endl << "Password: " << part5.theusername << endl;
	}
    system("pause");
    return 0;
}
...
void test(){
    ...
    */
    ...
    //int length = sizeof(mon.monarchArray)/sizeof(mon.monarchArray[0]);

    /*cout << sizeof(textKeys)/sizeof(textKeys[0]) << endl << sizeof(dateKeys)/sizeof(dateKeys[0]) << endl;
    int textIndex;
    int dateIndex;
    for (int i = 0; i < 1000; i++){
        textIndex = rand() % 10000;
        dateIndex = rand() % 10000;
        cout << endl << i << endl << endl << "Text Key: " << textKeys[textIndex] << endl << "Date Key: " << dateKeys[dateIndex] << endl;
        username test = username(getTextKey(textKeys[textIndex]), dateKeys[dateIndex]);
        password testpass = password(test.plaintext, test.ciphertext);
        monarch testmon = monarch(testpass.targetKey);
        cout << "Username: " << test.theusername << endl << "Password: " << testmon.password << endl;
    }
